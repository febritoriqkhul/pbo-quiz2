 ...
 */
package com.febri.transaksi;

import com.febri.library.ComboBoxModel;
import com.febri.library.Items;
import com.febri.library.TabelModel;
import com.febri.library.TransaksiItem;
...
public class TransaksiMain extends javax.swing.JFrame {
    ...
    private int id = 0; // variabel id untuk kode iterasi kode transaksi
    private String code; // variabel kode untuk kode transaksi
    private DefaultComboBoxModel cbModel; // variabel cmbModel untuk JcomboBox model
    private DefaultTableModel tbModel; // variabel tblModel untuk Jtable model
    private ArrayList<Items> cart = new ArrayList<>(); // variabel cart untuk menyimpan masing-masing transaksi item
    ...
    public TransaksiMain() {
        ...
        this.cbModel = new DefaultComboBoxModel<>(comboModel.getName().toArray()); // untuk mengatur combo item
        
        TabelModel tablemodel = new TabelModel();
        this.tbModel = new DefaultTableModel(tablemodel.getColumnName(), 0); // untuk mengatur nama kolom pada tabel
    ...
    private String setCode(){
        this.incId();
        String tgl = new SimpleDateFormat("yyMMdd").format(new Date()); // mendapatkan tanggal saat ini
        this.code = String.format(tgl+"%02d", this.id); // menggabungkan tanggal saat ini dengan id
        return code;
    }
    ...
    private Object[] addItem(String nama, int jumlah){
        ...
        for (int i = 0; i < items.getName().size(); i++) {
            if(nama.equalsIgnoreCase(items.getName().get(i))){
                harga = items.getPrice().get(i);
            }
        }
        Object[] object = {
            nama,
    ...
    private void updateJumlah(String nama, int tambah){
        ...
        for (int i = 0; i < item.size(); i++) {
            if(item.get(i).equals(nama)){
                ...
                tbModel.setValueAt(jumlah+tambah, i, 2);
            }
        }
    }
    
    // fungsi untuk mengecek item yang dipilih sudah terpilih di item sebelumnya
    ...
    // memasukkan transaksi baru setelah transaksi berhasil selesai
    private void newTrx(){
        this.txtJml.setText("");
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        try {
            for (int i = 0; i < tbModel.getRowCount(); i++) {                   
                ...
                int jumlah = new Integer(tbModel.getValueAt(i, 2).toString());
                this.cart.add(new Items(nama, harga, jumlah));
            }
            TransaksiItem Trx = new TransaksiItem(this.code, this.cart);
            StringBuilder str = new StringBuilder();
            str.append(Trx.tampilData());
